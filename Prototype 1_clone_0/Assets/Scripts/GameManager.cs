...
public class GameManager : MonoBehaviour, IOnEventCallback
{
    ...
    IEnumerator IdentifyLocation(int x, int z, TileInfo info)
    {
        ...
        {
            object[] content = new object[] {x, z, info.xPos, info.zPos};
            RaiseEventOptions raiseEventOptions = new RaiseEventOptions { Receivers = ReceiverGroup.Others };
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        if (eventCode == OnTileSelected)
        {
            ...
            int x = (int)data[0];
            int z = (int)data[1];
            int tileX = (int)data[2];
            int tileZ = (int)data[3];
            TileInfo info = new TileInfo(); //Temporary in order for this.IdentifyLocation to work
            //Get Tile Info here
            this.IdentifyLocation(x, z, info);
